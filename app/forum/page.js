  ...
  const handleBuatPost = async () => {
    ...
    if (!error) {
      await supabase.from('activity_logs').insert({
        user_id: profile.id,
        aksi: 'membuat topik forum',
        deskripsi: `"${form.judul}" di kategori ${form.kategori}`
      })
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="p-4 md:p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowForm(true)} className="bg-himmah-accent px-4 py-2 rounded-lg text-white flex items-center gap-2">
              <HiPlus /> Buat Topik
            </button>
          </div>
            ...
            {posts.map((post) => (
              <motion.div key={post.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs text-gray-400">{format(new Date(post.created_at), 'dd MMM yyyy', { locale: id })}</span>
                </div>
                ...
                <AnimatePresence>
                  {selectedPost?.id === post.id && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-4 pt-4 border-t border-himmah-medium">
                      <h4 className="text-white text-sm font-medium mb-2">Komentar</h4>
                      <div className="space-y-2 mb-3">
                        <p className="text-xs text-gray-400">Komentar akan muncul di sini.</p>
                      </div>
                      <div className="flex gap-2">
                        <input value={komentar} onChange={(e) => setKomentar(e.target.value)} placeholder="Tulis komentar..." className="flex-1 bg-himmah-medium text-white rounded-lg px-3 py-2 text-sm"
                          onKeyDown={(e) => e.key === 'Enter' && handleKomentar(post.id)} />
                        <button onClick={() => handleKomentar(post.id)} className="bg-himmah-accent px-3 py-2 rounded-lg text-white text-sm"><HiReply /></button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <AnimatePresence>
            {showForm && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
                <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="bg-himmah-dark p-6 rounded-2xl w-full max-w-md border border-himmah-accent/30">
                  <h3 className="text-xl font-bold text-white mb-4">Buat Topik Baru</h3>
                  <div className="space-y-3">
                    <input value={form.judul} onChange={(e) => setForm({ ...form, judul: e.target.value })} placeholder="Judul topik" className="w-full bg-himmah-medium text-white rounded-lg px-4 py-2" />
                    <select value={form.kategori} onChange={(e) => setForm({ ...form, kategori: e.target.value })} className="w-full bg-himmah-medium text-white rounded-lg px-4 py-2">
                      <option>Umum</option>
                    ...
                    <textarea value={form.konten} onChange={(e) => setForm({ ...form, konten: e.target.value })} placeholder="Isi topik..." rows={4} className="w-full bg-himmah-medium text-white rounded-lg px-4 py-2 resize-y" />
                  </div>
